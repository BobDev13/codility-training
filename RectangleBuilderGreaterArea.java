...
The pen will be rectangular and built from exactly four pieces of fence (so, the pieces of fence forming the opposite sides of the pen must be of equal length). 
Woolly can choose these pieces out of N pieces of fence that are stored in his barn. To hold the entire flock, 
the area of the pen must be greater than or equal to a given threshold X.
...
public class Solution55 {
    public static int solution(int[] A, int X){
        ...
        ArrayList<Integer> listaIndeksi = new ArrayList<Integer>();
        ArrayList<String> listaRazliki = new ArrayList<String>();
        String s,r,sr,rs = "";
        int brojac = 0;
        for(int i = 0; i < A.length; i++){
            for(int j = i+1; j < A.length; j++){
                if(A[i] == A[j] && !listaIndeksi.contains(j) && !listaIndeksi.contains(i)){
                    lista.add(A[i]);
                    listaIndeksi.add(j);
                    break;
                }
        ...
        for(int i = 0; i < lista.size(); i++){
            for(int j = i+1; j < lista.size(); j++){
                if(lista.get(i)*lista.get(j) >= X && !listaRazliki.contains(lista.get(i).toString()+lista.get(j).toString())){
                    brojac++;
                    s = lista.get(i).toString();
                    r = lista.get(j).toString();
                    sr = s+r;
                    rs = r+s;
                    listaRazliki.add(sr);
                    listaRazliki.add(rs);
                }
                if(brojac > 1000000000){
                    return -1;
                }
            }
        }
    ...
    public static void main(String[] args){
        int[] niza = {1,2,5,1,1,2,3,5,1};
        int broj =5;
        int izlez = solution(niza,broj);
