...
public class Solution54 {
    public static int solution(int K, int[] C, int[] D){
        ArrayList<Integer> preskokni = new ArrayList<Integer>();
        int brojacCisti = 0;
        ArrayList<Integer> ostanati = new ArrayList<Integer>();
        int flag = 0;
        int brojacOstanati = 0;
        for(int i = 0; i < C.length; i++){
            for(int j = i+1; j < C.length; j++ ){
                if(C[i] == C[j] && !preskokni.contains(i) && !preskokni.contains(j)){
                    brojacCisti++;
                    preskokni.add(j);
                    flag = 1;
                    break;
            ...
            if(flag == 0 && !preskokni.contains(i)){
                ostanati.add(C[i]);
                flag = 0;
            }
            flag = 0;
        ...
        preskokni.clear();
        for(int j = 0; j < ostanati.size(); j++){
            for(int a = 0; a < D.length; a++){
                if(K>=1 && ostanati.get(j) == D[a] && !preskokni.contains(a)){
                    K--;
                    brojacOstanati++;
                    preskokni.add(a);
                    break;
        ...
        if(K != 0){
            ...
            for(int i = 0; i < D.length; i++){
                for(int j = i+1; j < D.length; j++ ){
                    if(D[i] == D[j] && !preskokni.contains(i) && !preskokni.contains(j) && K > 1){
                        brojacOstanati++;
                        preskokni.add(j);
                        K-=2;
                        break;
        ...
        return brojacCisti+brojacOstanati;
    }

    public static void main(String[] args){
        int[] C = {1,2,1,3,4,1,2,1};
        int[] D = {1,4,3,2,4};
        int K = 2;
        int izlez = solution(K,C,D);
