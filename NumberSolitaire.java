...
A non-empty array A of N integers contains the numbers written on the squares. Moreover, some squares can be marked during the game.
...
Then we move the pebble standing on square number I to square number I + K, providing that square number I + K exists. If square number I + K does not exist, 
...
public class Solution42 {
    public static int solution(int[] A){
        ...
        for(int i = 1; i<A.length; i++){
            ...
            for(int j = 1; j<=6; j++){
                if(i-j >= 0){
                    maksimum = Math.max(niza[i-j]+A[i],maksimum);
                }
            }
            niza[i] = maksimum;
        }
        return niza[A.length-1];
    }


    public static void main(String[] args){
        int[] niza = {1,-2,0,9,-1,-2};
