...
public class Solution34 {
    public static int solution(int[] A){
        ...
        for(int i = 1; i < A.length; i++){
            //System.out.println(leftMax[i-1] + " + "+A[i]);
            leftMax[i] = Math.max(0,leftMax[i-1]+A[i]);
        }
        //System.out.println("Nova niza:");
        for(int i = A.length-2; i>=0; i--){
            //System.out.println(rightMax[i+1] +" + "+A[i]);
            rightMax[i] = Math.max(0,rightMax[i+1]+A[i]);
        }
        for(int i = 1; i < A.length-1; i++){
            if(leftMax[i-1]+rightMax[i+1] > maxDoubleS){
                maxDoubleS = leftMax[i-1] + rightMax[i+1];
            }
        }
        return maxDoubleS; 
    }
    public static void main(String[] args){
        int[] niza = {3,2,6,-1,4,5,-1,2};
