...
public class Solution43 {
    public static int solution(String E, String L){
        ...
        for(String a : E.split(":")){
            if(flag == 0){
                hhpocetok = Integer.parseInt(a);
                flag = 1;
            }
            else mmpocetok = Integer.parseInt(a);
        }
        flag =0;
        for(String b : L.split(":")){
            if(flag == 0){
                hhkraj = Integer.parseInt(b);
                flag = 1;
            }
            else mmkraj = Integer.parseInt(b);
        }
        hhrazlika = hhkraj - hhpocetok;
        mmrazlika = mmkraj - mmpocetok;
        if(hhrazlika == 0){
            return 2+3;
        }
        else{
            parkingsmetka = 2+3+(4*(hhrazlika-1));
        }
        if(mmrazlika > 0){
            parkingsmetka += 4;
        }
        return parkingsmetka;
    }
    public static void main(String[] args){
        String h = "09:42";
        String h1 = "11:40";
        int izlez = solution(h, h1);
