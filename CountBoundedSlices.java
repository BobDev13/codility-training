...
public class Solution58 {
    public static int solution(int K, int[] A){
        ...
        int mmaksimum = 0;
        int mminimum = Integer.MAX_VALUE;
        int brojac = 0;
        for(int i = 0; i < A.length; i++){
            for(int j = i+1; j < A.length; j++){
                mmaksimum = Math.max(A[i],A[j]);
                mminimum = Math.min(A[i],A[j]);
                maksimum = Math.max(maksimum,mmaksimum);
                minimum = Math.min(minimum,mminimum);
                if(maksimum - minimum <= K){
                    brojac++;
                }
                else{
                    mmaksimum = 0;
                    mminimum = Integer.MAX_VALUE;
                    maksimum = 0;
